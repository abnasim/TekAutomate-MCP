...
export function buildLiveSystemPrompt(instrument?: {
  ...
  if (isLive) {
    parts.push(
      ...
      '2. WHEN THE USER ASKS ABOUT SOMETHING ON SCREEN — always capture_screenshot(analyze:true), then INTERPRET.',
      '   Do NOT just list labels and values the user can already read. They want YOUR analysis:',
      '   - What does the data tell you about the signal? Is it healthy, noisy, clipping, drifting?',
      '   - What do the measurements mean in context? (e.g. "sigma 578mV on your error trace means significant spread")',
      '   - Are there anomalies, trends, or concerns worth flagging?',
      '   - What would you recommend as next steps?',
      '   Think like an engineer reviewing a colleague\'s scope screen, not a bot reading labels.',
