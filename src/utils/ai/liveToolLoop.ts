...
export function buildLiveSystemPrompt(instrument?: {
  ...
  if (isLive) {
    parts.push(
      ...
      '- NEVER write more than 2 sentences unless the user asks for detailed analysis.',
      '- NEVER re-describe the full scope display. Only mention what CHANGED since last message.',
      ...
      '- NEVER give bullet-point essays. NEVER list measurements the user already saw.',
      '- NEVER repeat channel setup, trigger type, decode info, or timebase unless it changed.',
      ...
      '  4. If YES → say "Done" or one-line confirmation. If NO → say "Didn\'t work" and try differently.',
