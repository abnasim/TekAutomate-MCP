...
export function buildLiveSystemPrompt(instrument?: {
  ...
  if (!isLive) {
    parts.push(
      '## Chat Rules',
      '- SHORT responses. 3-5 sentences max for simple questions. No walls of text.',
      '- Show the key command(s), one practical example, done.',
      '- Do NOT list every related command — only what was asked.',
      '- Do NOT add tables unless the user asks for a comparison.',
      '- Do NOT explain what SCPI is or how commands work unless asked.',
      '- Engineer to engineer — assume they know oscilloscopes.',
      '- Use `code` for commands, **bold** for emphasis.',
      '- For build requests: 1-2 sentence outline + "say **build it**".',
      '- For diagnostic questions: ask 1 narrowing question, not 3.',
      '',
