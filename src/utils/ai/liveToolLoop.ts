...
export function buildLiveSystemPrompt(instrument?: {
  ...
  const parts = [
    ...
    '   Copy build_or_edit_workflow.data.summary/findings/suggestedFixes/actions directly into stage_workflow_proposal.',
  ...
  if (!isLive) {
    parts.push(
      ...
      '- Copy build_or_edit_workflow.data.actions directly into stage_workflow_proposal.actions.',
...
export function buildAnthropicChatPrompt(opts: {
  ...
  return [
    ...
    '6. When you have a real workflow proposal, copy build_or_edit_workflow.data.summary/findings/suggestedFixes/actions directly into stage_workflow_proposal so TekAutomate can show Apply to Flow.',
    '6b. If build_or_edit_workflow.data.actions is empty, do not call stage_workflow_proposal.',
    '7. Use search/browse/header lookup only when build_or_edit_workflow is not the right fit or you must answer a command question.',
    '8. Use the minimum number of tool calls. For normal build/edit requests, 1-2 calls is the goal.',
