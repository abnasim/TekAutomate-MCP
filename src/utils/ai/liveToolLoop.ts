...
export function buildLiveSystemPrompt(instrument?: {
  ...
  if (isLive) {
    parts.push(
      ...
      '2. DO NOT ASK questions you can answer yourself. Before asking the user anything:',
      '   - Use capture_screenshot(analyze:true) to see the current scope state',
      '   - Check which channels are active, what measurements exist, what the timebase is',
      ...
      '3. MINIMUM TOOL CALLS. Simple tasks = 1-2 calls. "check scope" = capture_screenshot. "add freq measurement" = send_scpi.',
      '4. Common commands — send_scpi IMMEDIATELY: *RST, *IDN?, AUTOSet EXECute, MEASUrement:ADDMEAS <type>, CH<x>:SCAle, HORizontal:SCAle, TRIGger:A:EDGE:SLOpe',
      '5. Unknown commands — tek_router search_exec → send_scpi. Two calls max.',
      '6. Errors — read response, fix, retry. Briefly say what failed.',
      '7. capture_screenshot — ALWAYS after: ADDMEAS, scale/trigger/timebase changes, errors.',
      '   Default: no analyze. Pass analyze:true ONLY to read/diagnose the display.',
      '8. NEVER use discover_scpi unless search AND browse both failed AND the user confirms.',
      ...
      '9. Before adding measurements: MEASUrement:LIST? to check what exists.',
      '10. TIMEOUTS: Try *IDN? first. Do NOT retry same command repeatedly.',
      '',
    );
  }
