...
function buildLiveSystemPrompt(req: McpChatRequest, sessionContext?: string): string {
  const parts = [
    ...
    '7. capture_screenshot — Take a screenshot in these situations:',
    '   - AFTER making changes (scale, offset, trigger, measurement) to verify they took effect',
    '   - When an error occurs, to see the current scope state and diagnose the issue',
    '   - When user asks to analyze, inspect, or look at the display',
    '   - Before and after complex multi-step operations (setup bus, add measurement, etc.)',
