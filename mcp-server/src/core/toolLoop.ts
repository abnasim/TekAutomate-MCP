...
function buildLiveSystemPrompt(req: McpChatRequest, sessionContext?: string): string {
  const parts = [
    ...
    '',
    '### Execute on scope',
    '- **send_scpi** — Send SCPI commands. Check the "responses" array: {command, response, ok, error}. Queries (?) return values in "response".',
    '- **capture_screenshot** — See the scope display. Describe what you see.',
    '- **probe_command** — Test a single command before committing.',
    '- **get_instrument_state** — *IDN? and status.',
    '',
    '### Explore SCPI database (9,300+ commands)',
    '- **get_command_group** — Browse all commands in a group. Use group names: Vertical, Horizontal, Trigger, Measurement, Bus, Power, Display, Save and Recall, etc.',
    '- **get_command_by_header** — Full details for a specific command header.',
    '- **search_scpi** — Keyword search across all commands.',
    '- **smart_scpi_lookup** — Natural language search.',
    '- **list_command_groups** — See all available groups.',
    '',
    '### Learn & save',
    '- **save_learned_workflow** — After achieving a goal, save the successful command sequence as a reusable workflow. Provide: name, description, trigger phrases, and the steps that worked.',
    '',
    '### Knowledge',
    '- **retrieve_rag_chunks** — Search knowledge base (errors, app_logic, pyvisa_tekhsi, tmdevices).',
    '- **search_known_failures** — Troubleshooting known errors.',
    '',
    '## Rules',
