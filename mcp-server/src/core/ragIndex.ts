...
export class RagIndexes {
  ...
  search(corpus: RagCorpus, query: string, topK = 5, modelFamily?: string): RagChunkDoc[] {
    ...
    // Model-family filtering: hard-filter chunks tagged for a different family,
    // always pass through chunks with no model-specific tags (general content).
    // Applies to both tek_docs and tmdevices corpora.
    ...
    //   tek_docs:   productScore (family alias boost/penalty) + howToScore (FAQ boost)
    //   tmdevices:  family tag boost/penalty based on modelFamily or query alias
...
// ---------------------------------------------------------------------------
// Family tags are injected by buildRagIndex.ts from the "Models:" line in each
// docstring chunk body (e.g. "Models: MSO2, MSO4B, MSO5" → tags: ["MSO2","MSO4","MSO5"]).
// Chunks with no family tags are general content and always pass through.

/** All family tags that buildRagIndex injects into tmdevices chunks. */
const TM_DEVICES_FAMILY_TAGS = new Set([
  'MSO2', 'MSO4', 'MSO5', 'MSO6', 'LPD6',
  'MDO3K', 'MDO4K',
  'DPO4K', 'DPO5K', 'DPO7K', 'DPO70K',
  'AFG3K', 'AWG5200', 'AWG5K', 'AWG7K',
  'SMU', 'RSA',
]);

/** Expand a modelFamily string into the tm_devices family tags it maps to. */
function tmDevicesRequestedFamilies(modelFamily: string): Set<string> {
  ...
  if (out.size === 0) out.add(n); // pass through unknown families as-is
  return out;
}

/** Hard filter: exclude tmdevices chunks tagged for a different family. */
function tmDevicesFamilyMatches(doc: RagChunkDoc, modelFamily: string): boolean {
  ...
  if (familyTags.length === 0) return true; // no family tags → general content
  const requested = tmDevicesRequestedFamilies(modelFamily);
  return familyTags.some((t) => requested.has(t));
}

/** Soft scoring: +4 if chunk family matches requested/query family, -2 if wrong family. */
function scoreTmDevicesFamily(doc: RagChunkDoc, normalizedQuery: string, modelFamily?: string): number {
  const familyTags = (doc.tags || []).filter((t) => TM_DEVICES_FAMILY_TAGS.has(t));
  if (familyTags.length === 0) return 0; // general content, no adjustment

  // Determine which families are relevant from explicit modelFamily or query aliases
  const requested = new Set<string>();
  if (modelFamily) {
    for (const f of tmDevicesRequestedFamilies(modelFamily)) requested.add(f);
  }
  // Also pick up product aliases directly in the query (e.g. "mso24 fastacq")
  ...
  if (familyTags.some((t) => requested.has(t))) return 4;  // right family
